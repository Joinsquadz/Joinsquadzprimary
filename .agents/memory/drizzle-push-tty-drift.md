...
- **Columns specifically:** compare `lib/db/src/schema/users.ts` column list against `SELECT column_name FROM information_schema.columns WHERE table_name='users'`. Any column in Drizzle but not in the DB causes EVERY query on that table to 500 (Drizzle always SELECTs all columns). Fix: `ALTER TABLE users ADD COLUMN IF NOT EXISTS <col> <type> [DEFAULT]` for each missing one. This has happened with columns added by task-agent merges: `notify_payments`, `venmo_handle`, `cashapp_handle`, `zelle_handle`, `bio`, `hometown` were all missing at one point.
