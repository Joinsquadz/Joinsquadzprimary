...
**Two distinct phone errors — do not conflate.** "The certificate for this server is invalid …`repl.co`" = stale client cache / wrong URL (see above). **"Could not connect to the server" on the CORRECT `exp://…expo.kirk.replit.dev` URL = phone↔server network failure**, NOT a server/code bug. Prove the server is fine first: the `screenshot` tool with `type:external_url` on `https://$REPLIT_EXPO_DEV_DOMAIN/` uses Firecrawl (a genuinely external client) and, when healthy, returns the **fully rendered app** (home screen, tab bar) — that single shot proves (a) the domain is publicly reachable from outside Replit and (b) the app renders. Once that passes, remaining device failures are environmental (Apple iCloud Private Relay / VPN / weak cellular pulling the ~18MB dev bundle), and the durable iPhone fix is a real build via **Expo Launch** (suggestDeploy), which removes the dev-server dependency entirely.

**`app_preview` screenshots of this RN-Web app render all-white even when the app works.** Don't treat a blank `app_preview` as a broken app. Confirm via console (`Running application "main"` + `shadow*`/`useNativeDriver` warnings + real API calls firing = the tree mounted and painted) and via the external `screenshot type:external_url` which DOES capture the rendered UI.
